...
### Fixed
- **`doctor` schema version check no longer produces false positives on migrated databases.**
  `install.mjs` had a hardcoded `const codeVer = 1` that caused `doctor` to report
