 ...
 *   wicked.teststrategy.authored     — strategies.create
 *   wicked.scenario.authored         — scenarios.create
 *   wicked.testrun.started           — runs.create
 *   wicked.testrun.finished          — runs.update (finished_at set)
 *   wicked.verdict.recorded          — verdicts.create
 *   wicked.evidence.captured         — verdicts.create (vault_payload_sha present)
 *   wicked.contract.published        — vault CLI layer (bin/wicked-vault.mjs declare-contract)
 *
 * Note on wicked.contract.published: vault contracts are flat-file artifacts
 * managed by bin/wicked-vault.mjs, not DomainStore rows. The event is emitted
 * directly by the vault CLI via src/vault/bus.mjs (fire-and-forget) so it does
 ...
 *   artifacts managed by bin/wicked-vault.mjs and do not have a DomainStore
 *   row. That event is emitted directly by the vault CLI (fire-and-forget via
 *   src/vault/bus.mjs) and therefore never passes through this function.
...
export function domainEventToBusEvent(action, source, record, wickedTestingVersion) {
  ...
  if (action === "create") {
    switch (source) {
      ...
      case "verdicts":
        // wicked.evidence.captured fires alongside wicked.verdict.recorded when
        // the verdict was created with a vault_payload_sha (i.e. vault evidence
