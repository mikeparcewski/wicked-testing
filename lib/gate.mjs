...
function spawnBusEmit(type, domain, subdomain, payload, idempotencyKey) {
  ...
  // npx fallback (slower, but works without global install)
  try {
    spawnSync("npx", ["wicked-bus", ...args], { stdio: "pipe", timeout: 10000, shell: process.platform === "win32" });
...
export async function runGate({ projectId, runId, verdict, verdictSummary, rationaleRef, councilRunId, mode, dryRun = false }) {
  ...
  // 4. Write gate record to domain store (graceful degradation — bus event must still fire)
  ...
  if (!dryRun) try {
    ...
    store.create("verdicts", {
      ...
      ...(rationaleRef ? { equivalence_json: JSON.stringify({ rationale_ref: rationaleRef }) } : {}),
    });
  } catch (err) {
    process.stderr.write(`[wicked-qe] domain store write failed (non-fatal): ${err.message}\n`);
    // continue — do not abort the gate on a store failure
  } finally {
    if (store) {
