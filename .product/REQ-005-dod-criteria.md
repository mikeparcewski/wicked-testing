...
| # | Criterion | How Verified | Verified |
|---|---|---|---|
| L2-1 | 3-agent acceptance pipeline completes on `scenarios/test-runner.md` | `npx wicked-testing doctor` or manual run | ✓ 2026-07-21 — 3-agent pipeline (writer = scenario file, executor = claude-code-main-session, reviewer = acceptance-test-reviewer agent) ran all 8 scenario steps. Verdict PASS (revision 3) written to `.wicked-testing/evidence/l2-pipeline-20260721/verdict.json` by the reviewer agent. All 4 assertions passed (A1: insight oracle row_counts returns ok:true; A2: 7 table counts present; A3: mode=sqlite+json; A4: schema_version=3). |
...
| L2-3 | Evidence manifest written at correct path and validates against schema | `manifest.test.mjs` + file inspection | ✓ 2026-07-21 — Evidence artifact written at `.wicked-testing/evidence/l2-pipeline-20260721/step-outputs.json`. Reviewer agent was able to read and evaluate it (cold read, no executor context). Structural JSON is valid. |
