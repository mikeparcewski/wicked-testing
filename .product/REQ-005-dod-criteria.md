...
| L2-6 | SQLite degrades to JSON-only cleanly when `better-sqlite3` fails to load | `domain-store.test.mjs` mock path | — — `lib/domain-store.mjs` implements the fallback (lines 54–65: catches `require('better-sqlite3')` error, sets `this._db = null`, logs to stderr, continues in JSON-only mode). However, `domain-store.test.mjs` only asserts `store.mode === "sqlite+json"` — no test mocks the SQLite load failure or asserts JSON-only behaviour. Mock-path test is missing; criterion cannot be checked off from tests alone. |
