...
|---|---|---|---|
| L1-1 | All 48 skills are present under `skills/` | `ls skills/ \| wc -l` = 48 | ✓ 2026-07-21 — `npm run prepublishOnly` reports "plugin.json in sync (v0.9.0, 48 skills)". `ls skills/ \| wc -l` = 49 because `wicked-vault/` is a namespace directory without a top-level `SKILL.md`; `sync-plugin-version.mjs` correctly excludes it, yielding 48 active skills. |
...
| L1-4 | Unit tests pass | `npm test` exits 0 | ✓ 2026-07-21 — `npm test` exits 0. `npm run test:unit` runs 85 tests — 85 pass, 0 fail. |
...
| L1-6 | No ad-hoc SQL in any file (no string-concatenated SQL) | Code review + `grep` check | ✓ 2026-07-21 — Code review of `lib/oracle.mjs` confirms all SQL is static strings with `?` positional placeholders only. `oracle-queries.test.mjs` test "every query's SQL is a static string of SELECTs with ? placeholders only" asserts this programmatically and passes. |
...
| L2-6 | SQLite degrades to JSON-only cleanly when `better-sqlite3` fails to load | `domain-store.test.mjs` mock path | — `lib/domain-store.mjs` implements the fallback (lines 54–65: module-level `try/catch` around `require('better-sqlite3')` leaves `Database` as `null` on failure; `DomainStore` only calls `_initDb()` when `Database` is available, otherwise stays in JSON-only mode). However, `domain-store.test.mjs` only asserts `store.mode === "sqlite+json"` in the happy path — no test mocks the `better-sqlite3` load failure or asserts JSON-only behavior. Mock-path test is missing; criterion cannot be checked off from tests alone. |
