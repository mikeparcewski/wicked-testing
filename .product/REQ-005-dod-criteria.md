...
|---|---|---|---|
| L1-1 | All 48 skills are present under `skills/` | `npm run prepublishOnly` exits 0 reporting 48 skills | ✓ 2026-07-21 — `npm run prepublishOnly` reports "plugin.json in sync (v0.9.0, 48 skills)". Note: `ls skills/ \| wc -l` = 49 because `wicked-vault/` is a namespace directory without a top-level `SKILL.md`; `sync-plugin-version.mjs` correctly excludes it, yielding 48 active skills. |
